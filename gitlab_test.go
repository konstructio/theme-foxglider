...
func fakeGitLab(t *testing.T) *httptest.Server {
	// now-relative stamps: fixed dates rotted out of the activity window
	// (?hours=168) the week after they were written
	ts := func(agoSec int) string {
		return time.Now().UTC().Add(-time.Duration(agoSec) * time.Second).Format(time.RFC3339)
	}
	_ = ts
	t.Helper()
	...
	mux.HandleFunc("/api/v4/projects/1/pipelines", auth(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `[{"id":11,"status":"success","ref":"main","sha":"abc123","web_url":"http://gl/platform/alpha/-/pipelines/11","created_at":"`+ts(86400)+`","updated_at":"`+ts(86200)+`"}]`)
	}))
	mux.HandleFunc("/api/v4/projects/1/pipelines/11", auth(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `{"id":11,"status":"success","ref":"main","sha":"abc123","web_url":"http://gl/platform/alpha/-/pipelines/11","created_at":"`+ts(86400)+`","updated_at":"`+ts(86200)+`","started_at":"`+ts(86400)+`","finished_at":"`+ts(86200)+`","duration":190}`)
	}))
	mux.HandleFunc("/api/v4/projects/1/pipelines/11/jobs", auth(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `[{"name":"build","stage":"build","status":"success","started_at":"`+ts(86400)+`","finished_at":"`+ts(86280)+`","duration":110,"web_url":"http://gl/platform/alpha/-/jobs/101"}]`)
	}))
	mux.HandleFunc("/api/v4/projects/1/events", auth(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `[{"action_name":"pushed to","created_at":"`+ts(86460)+`","author":{"username":"john"},"push_data":{"ref":"main","commit_title":"feat: x","commit_count":1}}]`)
	}))
	mux.HandleFunc("/api/v4/projects/2/pipelines", auth(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `[{"id":21,"status":"failed","ref":"main","sha":"def456","web_url":"http://gl/themes/beta/-/pipelines/21","created_at":"`+ts(82800)+`","updated_at":"`+ts(82740)+`"}]`)
	}))
