...
A read-only dashboard for GitLab build metadata: every project in every
...
corresponding GitLab page. Borrows the Konstruct theme architecture
(embedded static frontend + tiny Go server) without using any theme-rpc
operations.
...
| `GITLAB_GROUPS` | *(empty = all visible)* | comma-separated group paths to limit scope |
